...
export class PropertyService {
    ...
    async calculateBrrrr(data: CreatePropertyDto) {
        ...
        function calculateBrrrrMetrics(dto: CreatePropertyDto) {
            // --- DEFINITIONS ---
            const allInCost = dto.purchasePrice + dto.rehabCost + dto.closingCost + dto.holdingCost;
            const initialCashInvested = dto.downPayment + dto.rehabCost + dto.closingCost + dto.holdingCost;
            ...
            const cashOut = refinanceLoanAmount - initialLoanAmount - dto.refinanceCost;
            ...
            const refiMortgage = (refinanceLoanAmount * refiMonthlyRate) / (1 - Math.pow(1 + refiMonthlyRate, -(dto.refinanceLoanTerm * 12)));

            // Synchronized Cash Flow
            const flow = getOfficialCashFlow(dto, refiMortgage);
            const postRefiCoC = cashLeftInDeal > 0 ? (flow.annual / cashLeftInDeal) * 100 : 0;

            // NOI for Cap Rate & DSCR
            const effectiveIncome = (dto.monthlyRent * 12) * (1 - dto.vacancyRate / 100);
            const noi = effectiveIncome - ((dto.annualPropertyTax + dto.annualInsurance + dto.annualUtilities + dto.annualOtherExpense) +
                ((dto.maintenanceRate + dto.managementRate + dto.capexRate) / 100 * effectiveIncome));
        ...
        const getDealScoreboard = (m: any) => {
            ...
            const breakdown = [
                { name: "Cash Flow", value: m.flow.annual, ...scoreLookup(m.flow.annual, 3000, 1200) },
                { name: "Post-Refi CoC", value: m.postRefiCoC, ...scoreLookup(m.postRefiCoC, 12, 6) },
                { name: "Cap Rate", value: m.capRate, ...scoreLookup(m.capRate, 8, 5) },
                { name: "DSCR", value: m.dscr, ...scoreLookup(m.dscr, 1.25, 1.0) },
                { name: "1% Rule (All-In)", value: m.onePercentRule, score: m.onePercentRule ? 10 : 0, status: m.onePercentRule ? "GOOD" : "BAD" }
            ];

            const totalScore = breakdown.reduce((sum, i) => sum + i.score, 0);
            const rating = totalScore >= 40 ? "GOOD DEAL" : totalScore >= 25 ? "AVERAGE DEAL" : "BAD DEAL";
    ...
    async generateTurnkeyReport(dto: CreatePropertyDto) {
        ...
        const monthlyMortgage = (loanAmount * monthlyRate) / (1 - Math.pow(1 + monthlyRate, -(dto.loanTerm * 12)));

        // Re-using standardized cash flow logic
        const annualIncome = dto.monthlyRent * 12;
        const effectiveIncome = annualIncome * (1 - dto.vacancyRate / 100);
        const totalExpenses = dto.annualPropertyTax + dto.annualInsurance + dto.annualUtilities + dto.annualOtherExpense +
            ((dto.maintenanceRate + dto.managementRate + dto.capexRate) / 100 * effectiveIncome);
        ...
        const breakdown = [
            ...
            { name: "1% Rule (All-In)", value: onePercentRule, score: onePercentRule ? 10 : 0, status: onePercentRule ? "GOOD" : "BAD" }
        ];
        ...
        return {
                ...
                incomeExpance: {
                    ...
                    netCashFlow: { monthly: Number(monthlyCashFlow.toFixed(2)), annual: Number(annualCashFlow.toFixed(2)) },
    ...
    async saveDataToDatabase(userId: string, dto: PropertyCalculationResponseDto) {

        const record = await this.prisma.propertyCalculation.create({
            data: {
                strategy: dto.strategy,
                stateAddress: dto.stateAddress,
                userId: userId,
                allInCost: dto.allInCost,
                initialCashInvested: dto.initialCashInvested,
                monthlyCashFlow: dto.monthlyCashFlow,
                cashOnCashReturn: dto.cashOnCashReturn,
                capRate: dto.capRate,
                dscr: dto.dscr,
                onePercentRule: dto.onePercentRule,
                netOperatingIncome: dto.netOperatingIncome,
                monthlyRent: dto.monthlyRent,
                annualRent: dto.annualRent,
                effectiveIncome: dto.effectiveIncome,
                ...
                scoreBoardStatus: dto.scoreBoardStatus,
                annualNetCashFlow: dto.annualNetCashFlow,
                annualNoi: dto.annualNoi,
                monthlyMortgage: dto.monthlyMortgage,
                breakdown: {
                    create: dto.breakdown
                        ?.filter(item =>
                            item.name !== undefined &&
